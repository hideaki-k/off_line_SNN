...
sim, options = get_simulator(("--plot-figure","Plot the simulation results to a file.",{"action":"stone_true"}),
                ("--debug", "Print debugging information"))
...
hh = sim.Populaton(1, sim.HH_cond_exp(i_offset=0.2), label="HH_cond_exp")
adexp = sim.Population(1, sim.IF_cond_exp(isfa_ista(i_offset=1.0),label="EIF_cond_exp_isfa_ista"))
adapt = sim.Population(1, sim.IF_cond_exp_gsfa_grr(i_offset=2.0),label="IF_cond_exp_gsfa_grr")
izh = sim.Population(1, sim.Izikevich(i_offset=0.01),label="Izhikevich")

all_neurons = cuba_exp + hh + adapt + izh
...
if options.plot_figure:
    ...
    Figure(
        ...
        Panel(izh.get_data().segments[0].filter(name='u')[0],
              ...
              title="responses of standard neuro model to current injection",
